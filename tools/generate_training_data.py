"""Generate training data for any voice using 1.7B-Base-bf16 voice cloning.

Usage:
  python generate_training_data.py --voice nora --ref-text "Oh wow, that actually worked!..."
  python generate_training_data.py --voice joe --ref-text "Oh hey, that's actually..."
  python generate_training_data.py --voice nora --ref-text "..." --start 200  # resume from clip 201

Expects voices/<voice>/ref.wav to exist.
Outputs to voices/<voice>/training-data/audio-original/ + train.jsonl.
"""
import argparse
import time
import os
...
def main():
    ...
    # Open JSONL in append mode — each entry written immediately after clip saves
