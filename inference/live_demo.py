...
CHECKPOINT = str(Path.home() / "Desktop/Files/AI/holler/checkpoints/katie-v6-4bit")
PORT = 8099
SAMPLE_RATE = 24000
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  input { width: 100%; padding: 16px 20px; font-size: 18px; border: 1px solid #333;
...
<body><div class="container">
  <h1>Holler</h1>
  <p class="sub">Qwen3-TTS 0.6B &middot; 4-bit &middot; Katie &middot; Apple Silicon</p>
...
input.addEventListener('keydown', async (e) => {
  ...
  try {
    ...
    status.textContent = '';
    status.className = '';
    stats.textContent = `TTFA: ${data.ttfa_ms}ms | Audio: ${data.audio_len_s.toFixed(1)}s`;
  } catch(err) {
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print(f"  TTFA={ttfa:.0f}ms total={total_ms:.0f}ms audio={audio_len_s:.1f}s | {text[:60]}", flush=True)
